...
class RoxyInterface:
    ...
    async def _generate_response(self, user_input: str, memory) -> str:
        ...
        repo_path_nested = "/opt/roxy/mindsong-juke-hub/mindsong-juke-hub"
        ...
        if "list" in user_lower and ("page" in user_lower or "file" in user_lower or "component" in user_lower):
            ...
            if not actual_repo_path:
                # Try /opt/roxy as fallback
                if os.path.exists("/opt/roxy"):
                    actual_repo_path = "/opt/roxy"
        ...
        if "remember" in user_lower or "what do you know" in user_lower:
            if facts:
                ...
                if real_facts:
                    ...
                    return "I'm still learning! I have some system test data, but I'd love to learn more about you and your work. Tell me something about yourself or the mindsong-juke-hub project!"
            else:
        ...
        return f"I understand you said: '{user_input}'. I'm processing your request. (LLM service may need attention - check logs)"
    ...
    async def _handle_repo_audit(self, user_input: str, user_lower: str) -> str:
        ...
        try:
                ...
                if not repo_path:
                    ...
                    try:
                        result = subprocess.run(
                            ['find', '/opt/roxy', '-maxdepth', '2', '-type', 'd', '-name', '*mindsong*'],
            ...
            # Default to /opt/roxy if no specific repo mentioned
            if not repo_path:
                if os.path.exists("/opt/roxy") and os.path.isdir("/opt/roxy"):
                    # Check if it's a git repo
                    if os.path.exists("/opt/roxy/.git"):
                        repo_path = "/opt/roxy"
            
            # Default to /opt/roxy/mindsong-juke-hub if it exists
            if not repo_path:
                default_path = "/opt/roxy/mindsong-juke-hub"
            ...
            if not repo_path or not os.path.exists(repo_path):
                return f"I couldn't find the repository. I checked:\n" + \
                       ...
                       f"\n\nPlease specify the full path, or make sure the repository exists at /opt/roxy/mindsong-juke-hub"
    ...
    async def _handle_repo_info(self, user_lower: str) -> str:
        ...
        try:
            ...
            if not os.path.exists(repo_path):
                return "I don't have information about the mindsong-juke-hub repository. It doesn't appear to exist at /opt/roxy/mindsong-juke-hub"
    ...
    async def _index_and_respond(self, repo_path: str, user_lower: str) -> str:
        ...
        try:
            ...
            return f"Error starting repository indexing: {e}. You can manually index with: python3 /opt/roxy/scripts/index_mindsong_repo.py"
