...
class ToolRegistry:
    ...
    async def execute_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            missing_args = [param for param in required_params if param not in arguments]
            if missing_args:
                return {
                    'success': False,
                    'tool': tool_name,
            ...
            if 'missing' in error_msg and 'required positional argument' in error_msg:
                ...
                import re
                match = re.search(r"argument: '(\w+)'", error_msg)
                missing_arg = match.group(1) if match else 'unknown'
                logger.error(f"Tool {tool_name} missing required argument: {missing_arg}")
                return {
                    'success': False,
                    'tool': tool_name,
                    'error': f'Missing required argument: {missing_arg}. Provided arguments: {list(arguments.keys())}'
                }
            raise
    ...
    def _list_files_handler(self, path: str, pattern: str = None, recursive: bool = True) -> Dict:
        ...
        try:
            ...
            # Try to find the actual path if it doesn't exist
            repo = Path(path)
            if not repo.exists():
                # Try nested path
                nested_path = Path(path) / Path(path).name
                if nested_path.exists():
                    repo = nested_path
                    logger.info(f"Using nested path: {nested_path}")
                else:
                    # Try common alternatives
                    alternatives = [
                        "/home/mark/mindsong-juke-hub",
                        "/home/mark/mindsong-juke-hub/mindsong-juke-hub",
                        str(Path.home() / "mindsong-juke-hub"),
                    ]
                    for alt in alternatives:
                        alt_path = Path(alt)
                        if alt_path.exists() and ("mindsong" in path.lower() or "juke" in path.lower()):
                            repo = alt_path
                            logger.info(f"Using alternative path: {alt_path}")
                            break
                    else:
                        return {'error': f'Path not found: {path}. Tried: {path}, {nested_path}, and alternatives'}
