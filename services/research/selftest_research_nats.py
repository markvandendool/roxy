#!/usr/bin/env python3
"""
SKYBEAM Selftest: Deep Research Agent NATS Integration
=======================================================
...
Usage: python3 selftest_research_nats.py
"""
...
from deep_research_agent import (
    generate_research, publish_to_nats, validate_schema, write_research,
    load_trends, TRENDS_FILE
)
...
async def run_selftest():
    ...
    print("=" * 60)
    print("  SELFTEST: Deep Research Agent NATS Integration")
    print("=" * 60)
    ...
    # Generate research
    print("[4] Generating research brief...")
    research = generate_research(trends_data, top_n=5, test_degraded=False)
    ...
    print("[5] Writing research...")
    write_research(research)

    print("[6] Publishing to NATS...")
    pub_ok = await publish_to_nats(research)
    if not pub_ok:
    ...
    if len(received_messages) == 1:
        ...
        print(f"  Brief: {msg.get('brief_id')}")
        print(f"  Status: {msg.get('status')}")
        print(f"  Briefs: {msg.get('brief_count')}")
        print("\n" + "=" * 60)
        print("  RESULT: PASS")
        print("=" * 60)
        return True
